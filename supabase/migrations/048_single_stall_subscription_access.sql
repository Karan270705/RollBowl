-- ==============================================================================
-- Migration: 048_single_stall_subscription_access.sql
-- Description:
--   1. Removes college-scoped restrictions from create_subscription_purchase_request
--      (no longer rejects users when users.college_id is null or differs from stalls.college_id).
--   2. Enforces single-stall authoritative resolution:
--      - Resolves configured primary stall from authoritative database configuration
--        source (public.payment_settings where is_active = true) without inferring
--        primary status from created_at.
--      - Validates p_stall_id:
...
--   3. Preserves all Migration 047 financial, snapshot, uniqueness, and security logic.
--
-- PROPOSED SMALLEST SAFE MIGRATION FOR EXPLICIT PRIMARY DESIGNATION:
--   ALTER TABLE public.stalls ADD COLUMN IF NOT EXISTS is_primary BOOLEAN NOT NULL DEFAULT false;
--   CREATE UNIQUE INDEX IF NOT EXISTS idx_stalls_one_primary ON public.stalls (is_primary) WHERE is_primary = true;
--   UPDATE public.stalls SET is_primary = true WHERE id = '57a11000-0000-0000-0000-000000000001';
-- ==============================================================================
