-- ============================================================
-- RollBowl Migration 022: Notifications RPC
-- ============================================================
-- Provides a SECURITY DEFINER function so server-side operations 
-- (like the Kitchen app accepting an order) can insert a notification 
-- for a customer without violating RLS.
-- ============================================================
...
BEGIN
  ...
  IF auth.uid() != p_user_id THEN
    ...
    -- If the caller is not a kitchen operator or admin, reject the insert
    IF v_caller_role NOT IN ('kitchen', 'admin') THEN
...
COMMENT ON FUNCTION create_notification IS 'Securely creates a notification, enforcing caller role authorization.';
